    ...
    reducers: {
        ...
        revealTile: (state, {payload: startingCoords}: PayloadAction<Coordinates>) => {
            ...
            while (stack.length) {
                ...
                if (tile.state === 'REVEALED' || tile.state === 'FLAGGED') continue;
        ...
        incrementTileState: (state, {payload: {x, y}}: PayloadAction<Coordinates>) => {
            ...
            const currentFlagCount = state.grid.flat().filter(x => x.state === 'FLAGGED').length;

            if (currState >= 0) {
                const nextState = states[(currState + 1) % states.length];
                if (nextState !== 'FLAGGED' || currentFlagCount < state.minesCount || state.grid[y][x].state === 'FLAGGED') {
                    state.grid[y][x].state = nextState;
                }
            }
        }
    }
