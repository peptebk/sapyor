...
export default function Board({ size, grid, onRestart, onMsg, won, dead, last, startTime, endTime }: BoardProps) {
    ...
    useEventListener('mousedown', e => {
        const targetCell = data.dom2cell.get(e.target as HTMLElement) ?? null
        if (targetCell) {
        ...
        switch (data.buttons) {
            ...
            case 4:
                if (data.cell != null) {
                    onMsg({ type: 'preview', pos: data.cell.pos, cell: data.cell })
                }
                break;
        }
    })
    ...
    const face: Face = won ? 'won' : dead ? 'dead' : (data.buttons === 1 && data.cell && !data.cell.hasFlag) ? 'click' : 'normal'
